    ...
    public class GunGeneration
    {
        ...
        public class Gun
        {
            ...
            private void RecalculateStats()
            {
                ...
                Damage = 10 + DetermineStatBonus(UpperReceiver.Split('.')[1])
                            + DetermineStatBonus(Barrel.Split('.')[1])
                            + DetermineStatBonus(LowerReceiver.Split('.')[1])
                            + DetermineStatBonus(BufferTube.Split('.')[1])
                            + DetermineStatBonus(Stock.Split('.')[1])
                            + DetermineStatBonus(Grip.Split('.')[1])
                            + DetermineStatBonus(Trigger.Split('.')[1]);
                ...
                Accuracy = 50 + DetermineStatBonus(UpperReceiver.Split('.')[1])
                              + DetermineStatBonus(Barrel.Split('.')[1])
                              + DetermineStatBonus(LowerReceiver.Split('.')[1])
                              + DetermineStatBonus(BufferTube.Split('.')[1])
                              + DetermineStatBonus(Stock.Split('.')[1])
